...
import { usarSalaDeLaFicha } from './SeguirFicha'
...
export function Publicacion({
  ...
  // Las dos cosas que se deciden acá le cambian la pantalla a quien tenga la
  // ficha abierta: publicar le congela los parámetros y una versión nueva le
  // mueve el id al que van a parar sus guardados.
  const { avisar } = usarSalaDeLaFicha()
  ...
  const versionar = () => {
    ...
    arrancar(async () => {
      ...
      // Primero el aviso y después la navegación: en cuanto esta pantalla se
      // vaya a la versión nueva deja la sala, y desde afuera ya no hay a quién
      // avisarle.
      avisar()
      router.push(`/propuestas/${resultado.propuestaId}`)
    })
  }
