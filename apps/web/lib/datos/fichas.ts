...
export async function cargarRevision(fichaId: string): Promise<EstadoRevision | null> {
  ...
  const { data: propuesta } = await supabase
    .from('proposals')
    .select(
      'id, estado, perfil, institucional_override, toggle_inm_seccion_propia, ' +
        'accede_inmobiliario, fx, ' +
...
export async function cargarObjetivo(fichaId: string): Promise<ObjetivoCompartido | null> {
  ...
  const { data: propuesta } = await supabase
    ...
    .select(
      'id, estado, perfil, institucional_override, toggle_inm_seccion_propia, ' +
        'accede_inmobiliario, fx, colchon_liquidez_usd, ticket_minimo_etf_usd',
    )
    .eq('ficha_id', fichaId)
    .order('created_at', { ascending: false })
    .limit(1)
