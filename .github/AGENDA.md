...
- [ ] **`npm run lint` no corre.** El script está declarado en `package.json`
      pero eslint no está instalado ni hay configuración. Instalarlo con una
      config plana acorde a TypeScript estricto y a Next, dejar el repositorio
      sin hallazgos y agregar el paso a `.github/workflows/ci.yml`.
