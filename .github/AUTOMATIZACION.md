...
el CI que los vigila (`verificar.yml`, que ya vivía en `master` antes que el
resto y por eso no se llama `ci`). La idea es simple: dejas un issue asignado antes de
dormir y a la mañana hay un pull request esperando, aunque tu computadora haya
estado apagada toda la noche.

| Workflow | Cuándo corre | Qué hace |
|---|---|---|
| `verificar.yml` | cada pull request, y cada push a `master` o a `claude/**` | typecheck, lint y tests |
